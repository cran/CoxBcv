...
#' 
#' Calculate the Morel, Bokossa, and Neerchal (MBN; 2003) bias-corrected sandwich variance estimator, for marginal Cox analysis of cluster randomized trials, 
...
#'   \item MBN-var - MBN bias-corrected sandwich variance estimate of coef.
#' }
#' 
#' @export
#' 
...
CoxBcv.mbn <- function(Y,Delta,X,ID){
  ...
  ######################################################
  # Step 3: MBN and bias correction
  ######################################################
  ...
  #############################################
  # Output
  # varMBN: MBN bias-corrected sandwich var
  #############################################
  bvarMBN <- diag(varMBN)
  outbeta <- cbind(matrix(summary(test.cox_cluster)$coefficients[,1:2],ncol=2),bvarMBN)
  colnames(outbeta) <- c("coef","exp(coef)","MBN-var")
