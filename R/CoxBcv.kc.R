...
CoxBcv.kc <- function(Y,Delta,X,ID){
  ...
  #############################################
  # Output
  # varKC: KC bias-corrected sandwich var
  #############################################
  bvarKC <- diag(varKC)
  outbeta <- cbind(matrix(summary(test.cox_cluster)$coefficients[,1:2],ncol=2),bvarKC)
  colnames(outbeta) <- c("coef","exp(coef)","KC-var")
