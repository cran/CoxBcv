...
#' 
#' Calculate the martingale residual (MR) bias-corrected sandwich variance estimator, for marginal Cox analysis of cluster randomized trials,
...
#'   \item MR-var - MR bias-corrected sandwich variance estimate of coef.
#' }
#' 
#' @export
#' 
...
CoxBcv.mr <- function(Y,Delta,X,ID){
  ...
  ######################################################
  # Step 3: martingale residual based bias correction
  ######################################################
  ...
  #############################################
  # Output
  # varMR: MR bias-corrected sandwich var
  #############################################
  bvarMR <- diag(varMR)
    outbeta <- cbind(matrix(summary(test.cox_cluster)$coefficients[,1:2],ncol=2),bvarMR)
  colnames(outbeta) <- c("coef","exp(coef)","MR-var")
