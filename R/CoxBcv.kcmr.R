...
#' 
#' Calculate the hybrid KCMR bias-corrected sandwich variance estimator, for marginal Cox analysis of cluster randomized trials, 
#' proposed by Wang et al. (under review). KC: Kauermann and Carroll (2001); MR: martingale residual.
#' 
...
#'   \item KCMR-var - KCMR bias-corrected sandwich variance estimate of coef.
#' }
#' 
#' @export
#' 
...
CoxBcv.kcmr <- function(Y,Delta,X,ID){
  ...
  nom <- nomMR <- nomMRMD <- zeros(nbeta,n)
  ...
  for(i in UID){
    ...
    # components for KC type correction
    nomMRMD[,i] <- Ustar%*%solve(Ustar-Omega_m[,,i])%*%nomMR[,i]
  }
  
  # variance estimator of KCMR type
  UUKCMR <- tcrossprod(nomMRMD,nomMR)
  varKCMR <- naive%*%(UUKCMR+t(UUKCMR))%*%naive/2
  
  #############################################
  # Output
  # varKCMR: KCMR bias-corrected sandwich var
  #############################################
  bvarKCMR <- diag(varKCMR)
  outbeta <- cbind(matrix(summary(test.cox_cluster)$coefficients[,1:2],ncol=2),bvarKCMR)
  colnames(outbeta) <- c("coef","exp(coef)","KCMR-var")
