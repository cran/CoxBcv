...
#' 
#' Calculate the hybrid FGMR bias-corrected sandwich variance estimator, for marginal Cox analysis of cluster randomized trials, 
#' proposed by Wang et al. (under review). FG: Fay and Graubard (2001); MR: martingale residual.
#' 
...
#'   \item FGMR-var - FGMR bias-corrected sandwich variance estimate of coef.
#' }
#' 
#' @export
#' 
...
CoxBcv.fgmr <- function(Y,Delta,X,ID){
  ...
  nom <- nomMR <- nomFGMR <- zeros(nbeta,n)
  ...
  for(i in UID){
    ...
    nomFGMR[,i] <- Hi%*%nomMR[,i]
  }
  
  # variance estimator of FGMR type
  UUFGMR <- tcrossprod(nomFGMR)
  varFGMR <- naive%*%UUFGMR%*%naive
  
  #############################################
  # Output
  # varFGMR: FGMR bias-corrected sandwich var
  #############################################
  bvarFGMR <- diag(varFGMR)
  outbeta <- cbind(matrix(summary(test.cox_cluster)$coefficients[,1:2],ncol=2),bvarFGMR)
  colnames(outbeta) <- c("coef","exp(coef)","FGMR-var")
